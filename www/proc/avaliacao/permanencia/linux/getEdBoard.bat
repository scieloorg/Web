...
$MX "seq=$JOURNAL_EDBOARD_HTML_PATH/$ACRON/$FILENAME¨" lw=999999 "pft=if (v1:'<li>' or v1:'</li>') and not v1:'<a href' then replace(s(replace(replace(v1,'<li>',s(#,'<li>')),'</li>',s(#)),' '),'  ',' ') fi" now> $OUTPUT_PATH/$ACRON/edboard.tmp
