...
if [ "$SUBJ" == "ALL" ]
then
	./report/issue.sh $MX $CIPFILE $TEMP_PATH HR=S$ $PID_LANG.seq
else
	...
	$MX seq=$TEMP_PATH/issn.seq lw=9999 "pft='./report/issn.sh $MX $CIPFILE $TEMP_PATH $PID_LANG ',v1,' \"$SUBJ\" '/"  now > $TEMP_PATH/artigo.sh
...
then
	...
	echo For the list of PID vs language, generate PID vs combination of languages
	...
	echo  List of languages and their combinations
	...
	echo  invert
	$MX $YEAR_LANG fst=@report/year_lang.fst fullinv=$YEAR_LANG
	$MX $YEAR_LANG_COMB fst=@report/year_lang.fst fullinv=$YEAR_LANG_COMB
	$MX $TOTAL_ARTICLE_VERSIONS fst=@report/year_lang.fst fullinv=$TOTAL_ARTICLE_VERSIONS
	$MX $TOTAL_ARTICLE fst=@report/year_lang.fst fullinv=$TOTAL_ARTICLE
	...
	$MX $LANGS gizmo=report/gizmo,9000  lw=9999 "pft='./report/table.sh $MX ${YEARS}.seq $YEAR_LANG ',v1,' ',v9000,#" now           >  $TEMP_PATH/table.sh
	$MX null count=1      lw=9999 "pft='./report/table.sh $MX ${YEARS}.seq $TOTAL_ARTICLE_VERSIONS ',#" now >> $TEMP_PATH/table.sh
	$MX seq=${LANGS}1.seq "proc='a9000~',v1,'~'" create=${LANGS}1 now -all
	$MX ${LANGS}1 gizmo=report/gizmo,9000 lw=9999 "pft='./report/table.sh $MX ${YEARS}.seq $YEAR_LANG_COMB ',v1,' ',v9000,#" now      >> $TEMP_PATH/table.sh
	$MX null count=1      lw=9999 "pft='./report/table.sh $MX ${YEARS}.seq $TOTAL_ARTICLE ',#" now          >> $TEMP_PATH/table.sh
