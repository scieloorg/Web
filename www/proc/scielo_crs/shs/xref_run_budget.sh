...
$cisis_dir/mx cipar=$MYCIPFILE DB_PRESUPUESTOS btell=0 "$BUDGETID" "pft='fecha del presupuesto: 'v3/,'valor: ',v2/" now 
echo
BATCHBGID=`cat temp`
...
then
	...
	$cisis_dir/mx cipar=$MYCIPFILE DB_BATCH_RUN_BUDGET btell=0 "bool=BATCHBG=$BATCHBGID" "proc='a200{$FIRST_YEAR_OF_RECENT_FEE{a201{$RECENT_FEE{a202{$BACKFILES_FEE{'" copy=DB_BATCH_RUN_BUDGET now -all
