...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="AddScieloLink">
		...
		<xsl:param name="date"/>
		<xsl:param name="page"/>
		<xsl:param name="lang" select="$interfaceLang"/>
		<xsl:choose>
			<xsl:when test="$script = 'sci_pdf' ">
				<xsl:attribute name="href">javascript: void(0); </xsl:attribute>
				<xsl:attribute name="onClick">setTimeout("window.open('http://<xsl:value-of select="//CONTROLINFO/SCIELO_INFO/SERVER"/><xsl:value-of select="//CONTROLINFO/SCIELO_INFO/PATH_DATA"/>scielo.php?script=<xsl:value-of select="$script"/>&amp;<xsl:if test="$seq">pid=<xsl:value-of select="$seq"/>&amp;</xsl:if>lng=<xsl:value-of select="normalize-space($lang)"/>&amp;nrm=<xsl:value-of select="normalize-space(//CONTROLINFO/STANDARD)"/><xsl:if test="$txtlang">&amp;tlng=<xsl:value-of select="normalize-space($txtlang)"/></xsl:if><xsl:if test="$file">&amp;file=<xsl:value-of select="$file"/></xsl:if> ','_self')", 3000);</xsl:attribute>
			</xsl:when>
			<xsl:otherwise>
				<xsl:attribute name="href"><xsl:call-template name="getScieloLink"><xsl:with-param name="lang" select="$lang"/><xsl:with-param name="seq" select="$seq"/><xsl:with-param name="script" select="$script"/><xsl:with-param name="txtlang" select="$txtlang"/><xsl:with-param name="file" select="$file"/><xsl:with-param name="date" select="$date"/><xsl:with-param name="page" select="$page"/></xsl:call-template></xsl:attribute>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<xsl:template name="getScieloLink">
		...
		<xsl:param name="page"/>
		<xsl:param name="lang" select="$interfaceLang"/>
		<xsl:value-of select="$HOME_URL"/>?script=<xsl:value-of select="$script"/>&amp;<xsl:if test="$seq">pid=<xsl:value-of select="$seq"/>&amp;</xsl:if>lng=<xsl:value-of select="normalize-space($lang)"/>&amp;nrm=<xsl:value-of select="normalize-space(//CONTROLINFO/STANDARD)"/>
