        ...
        <tr>
			...
			<b>
				<xsl:choose>
					...
					<xsl:when test="CONTROLINFO/LANGUAGE = 'es' ">Revista (# visitas)</xsl:when>
					<xsl:when test="CONTROLINFO/LANGUAGE = 'pt' ">Revista (# visitas)</xsl:when>
				</xsl:choose>
			</b>
	     </td>
...
<xsl:template name="MONTHS">
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			<xsl:choose>
				<xsl:when test="MONTH[@NUMBER = $current]"><xsl:apply-templates select="MONTH[@NUMBER = $current]" /></xsl:when>
