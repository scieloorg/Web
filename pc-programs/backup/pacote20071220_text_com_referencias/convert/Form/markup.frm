        ...
        If SetSelectedDir(PathCurr, FileCounter) Then
            ...
            If Not (journalDirstructure.relatedissues.archive Is Nothing) Then
                If DirExist(journalDirstructure.relatedissues.archive.getMarkupFile()) Then
                    ...
                    ListDoc.Path = journalDirstructure.relatedissues.archive.getMarkupFile()
                    ArchiveFilesCounter = ListDoc.ListCount
                End If
            End If
            
            ListDoc.Path = PathCurr + .FileTree.DirNodes("Markup Directory").text
            totalOfFiles = ListDoc.ListCount + ArchiveFilesCounter
            
            LabDocCounter.Caption = InterfaceLabels("LabFileCount").elem2 + CStr(ListDoc.ListCount)
            If Not (journalDirstructure.relatedissues.archive Is Nothing) Then
                LabDocCounter.Caption = journalDirstructure.relatedissues.Issue.issueDir + ": " + CStr(ListDoc.ListCount) + "/" + CStr(totalOfFiles)
    ...
    If SelCount > 0 Then
        ...
        For i = 1 To SelCount
            ...
            If isBodyOK And isBaseNameOK Then
                'Make the conversion
                If MakeConversion(ListDoc.List(FileIndex(i)), BodyFile, BaseFile) Then
                    isConversionOK = True
                End If
            Else
...
Private Sub CmdHelp_Click()
    Call frmFindBrowser.CallBrowser(ConvertDirTree.DirNodes("Help of Markup").Parent.fullpath, ConvertDirTree.DirNodes("Help of Markup").text)
End Sub
