...
Property Let CurrImgIdx(idx As Long)
    ...
    If (idx > 0) And (idx <= ImgCount) Then
        Call SeparateFileandPath(ImgShortAddr(idx), Path, File)
        If FileExist(Path, File, "Image to replace") Then
...
Sub setImages(ImageAddress() As String, ImageShortAddress() As String, ImageReplace() As String, ImageCounter As Long)
    ...
    For i = 1 To ImageCounter
        ...
        If Len(ImageReplace(i)) > 0 Then
            ImgRepl(i) = ImageReplace(i)
        Else
            ImgRepl(i) = ImageAddress(i)
        End If
    Next
    CurrImgIdx = 1
    ...
    For i = 1 To ImgCount
        Call SeparateFileandPath(ImgShortAddr(i), path1, File1)
        If FileExist(path1, File1, "Image to replace") Then
            Call ImgList.ListImages.Add(i, "img" + CStr(i), LoadPicture(ImgShortAddr(i)))
        End If
    Next
    
    ChooseImagetoReplace
    ...
    If OKChoice Then
        ...
        For i = 1 To ImgCount
            ...
            ReDim Preserve ImageAddress(ImageCounter)
            ReDim Preserve ImageShortAddress(ImageCounter)
            ...
            Else
                ImageReplace(ImageCounter) = ImgAddr(i)
            End If
        Next
    Else
        ImageCounter = 0
...
Private Sub ChooseImagetoReplace()
    ListImages
