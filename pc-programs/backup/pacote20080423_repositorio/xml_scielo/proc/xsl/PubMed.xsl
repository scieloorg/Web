...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="record">
		<Article>
			...
			<xsl:if test=".//aheadOfPrint_oldpid[occ]">
				<Replaces IdType="pii">
					<xsl:apply-templates select="." mode="pii"/>
				</Replaces>
			</xsl:if>
			<xsl:apply-templates select="title/occ" mode="title"/>
			<xsl:apply-templates select="title/occ" mode="titles"/>
			...
			<xsl:if test=".//received_dateiso[occ] or .//accepted_dateiso[occ] or .//aheadOfPrint_oldpid[occ]">
				<History>
					<xsl:apply-templates select=".//received_dateiso"/>
					<xsl:apply-templates select=".//accepted_dateiso"/>
					<xsl:if test=".//aheadOfPrint_oldpid[occ] and .//aheadOfPrint_date[occ]">
						<PubDate PubStatus="aheadofprint">
							<xsl:apply-templates select=".//aheadOfPrint_date" mode="data"/>
						</PubDate>
					</xsl:if>
				</History>
			</xsl:if>
	...
	<xsl:template match="record" mode="pii">
		<xsl:choose>
			<xsl:when test=".//aheadOfPrint_oldpid[occ]">
				<xsl:apply-templates select=".//aheadOfPrint_oldpid/occ"/>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="publishing_dateiso">
		<xsl:choose>
			<xsl:when test="../aheadOfPrint_date[occ] and not(../aheadOfPrint_oldpid[occ])">
