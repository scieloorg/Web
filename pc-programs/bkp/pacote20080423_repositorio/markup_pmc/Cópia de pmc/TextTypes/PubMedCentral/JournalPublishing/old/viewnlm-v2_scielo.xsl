...
<xsl:transform version="1.0" id="ViewNLM-v2-04.xsl" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:util="http://dtd.nlm.nih.gov/xsl/util" xmlns:mml="http://www.w3.org/1998/Math/MathML" exclude-result-prefixes="util xsl">
	...
	<xsl:variable name="person-strings" select="document('TextViewer/xsl/TextTypes/PubMedCentral/JournalPublishing/viewnlm-v2.xsl')/*/util:map[@id='person-strings']/item"/>
...
-->
  ...
  -->
	...
	<xsl:template name="make-href">
		<xsl:if test="@xlink:href"><!-- FIXED trocou src por href -->
	...
	<xsl:template match="mml:math" ><xsl:comment>viewnlm-v2.xsl</xsl:comment>
		<xsl:choose>
			<xsl:when test="@xlink:href">
				<table border="1">
					<tr>
...
-->
	...
	<xsl:template match="surname" mode="contrib"><xsl:comment>surname,contrib</xsl:comment>
		<xsl:apply-templates/>
	</xsl:template>
	...
	<xsl:template match="name">
		...
		<xsl:choose>
			<xsl:when test="given-names"><xsl:comment>name, when</xsl:comment>
				<xsl:apply-templates select="surname"/>&#160;<xsl:apply-templates select="given-names"/>
				<xsl:if test="suffix">
					&#160;<xsl:apply-templates select="suffix"/>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise><xsl:comment>name, otherwise</xsl:comment>
	...
	<xsl:template match="ref/citation">
		...
		<xsl:choose>
           ...
           = transform will supply it. -->
			...
			<xsl:comment>not(text()[normalize-space()])</xsl:comment>
...
-->
	...
	<xsl:template match="name" mode="book">
		...
		<xsl:choose>
			<!-- if given-names -->
			<xsl:when test="given-names"><xsl:comment>name, book</xsl:comment>
			...
			<!-- if no given-names -->
			<xsl:otherwise><xsl:comment>name, book, otherwise</xsl:comment>
