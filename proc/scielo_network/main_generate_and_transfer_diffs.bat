...
rem Obtém IN_SCIELO_NETWORK.txt
call scielo_network/transf.bat ${LOGFILE} ${TMP_PATH} IN_SCIELO_NETWORK.txt bin get


call batch/VerifExisteArquivo.bat ${TMP_PATH}/IN_SCIELO_NETWORK.txt
if [ -f ${TMP_PATH}/IN_SCIELO_NETWORK.txt ]
then

    rem Gera uma base isis SELECTED_DOCS e a indexa
    cisis/mx seq=${TMP_PATH}/IN_SCIELO_NETWORK.txt create=${TMP_PATH}/SELECTED_DOCS now -all
    ...
    cisis/mx ${ARTIGO} btell=0 tp=h lw=9999 "pft=if l(['${TMP_PATH}/SELECTED_DOCS']v880,ref(mfn-1,v91)) = 0 then v880/ fi" now > ${TMP_PATH}/NOT_IN_SCIELO_NETWORK.txt

    rem Gera as bases e arquivos id dos documentos que são novos ou desatualizados 
    cisis/mx seq=${TMP_PATH}/NOT_IN_SCIELO_NETWORK.txt lw=9999 "pft=if p(v1) then './scielo_network/generate_id_from_article.bat ${TMP_PATH} ${BASES_PATH} IV=',v1,'$ artigo_',v1/ fi" now > ${TMP_PATH}/NOT_IN_SCIELO_NETWORK.bat
    ...
    cisis/mx ${ARTIGO} btell=0 tp=h lw=9999 "pft=v880*0.14/" now | sort -u > ${TMP_PATH}/NOT_IN_SCIELO_NETWORK.txt

    rem Gera as bases e arquivos id dos documentos que são novos ou desatualizados 
    cisis/mx seq=${TMP_PATH}/NOT_IN_SCIELO_NETWORK.txt lw=9999 "pft=if p(v1) then './scielo_network/generate_id_from_article.bat ${TMP_PATH} ${BASES_PATH} IV=',v1*0.14,'$ artigo_',v1*0.14/ fi" now > ${TMP_PATH}/NOT_IN_SCIELO_NETWORK.bat
    chmod +x ${TMP_PATH}/NOT_IN_SCIELO_NETWORK.bat
    nohup ${TMP_PATH}/NOT_IN_SCIELO_NETWORK.bat > ${TMP_PATH}/nohup.NOT_IN_SCIELO_NETWORK.out&
...
cisis/mx seq=${TMP_PATH}/NOT_IN_SCIELO_NETWORK.txt lw=9999 "pft=v1*1.17/" now | sort -u > ${TMP_PATH}/ISSUE_NOT_IN_SCIELO_NETWORK.txt
cisis/mx seq=${TMP_PATH}/ISSUE_NOT_IN_SCIELO_NETWORK.txt lw=9999 "pft=if p(v1) then './scielo_network/generate_id_from_article.bat ${TMP_PATH} ${BASES_PATH} Y',v1,'$ i_',v1/ fi" now > ${TMP_PATH}/ISSUE_NOT_IN_SCIELO_NETWORK.bat
