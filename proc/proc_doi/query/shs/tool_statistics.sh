...
$MXTB $QUERYDB create=$BATCH_TEMP_PATH/numbers "100:'total_doi'/,(if size(v880^*)=23 or size(v880^*)=28 then 'pid_done' fi/),if p(v237) then 'doi'/ fi" class=1000000
...
$MXTB $QUERYLOGDB create=$BATCH_TEMP_PATH/numbers "100:'total_log'/,(if size(v880)=23 or size(v880)=28 then 'pid_toDo' fi/),if p(v100) then |status=|v100/ fi" class=1000000
$MX $BATCH_TEMP_PATH/numbers "pft='|',v1,'|',v999,'|'/" now >> $BATCH_LOGFILE
