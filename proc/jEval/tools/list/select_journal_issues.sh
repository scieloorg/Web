...
then
    ...
    $MX cipar=$FILE_CIPAR ISSUE btell=0 "seq=$PARM_ISSN$" lw=9999  "pft=if a(v131) and a(v132) and v32<>'ahead' and v32<>'review' and instr(v32,'spe')=0 and a(v41) then v35,v36*0.4,s(f(10000+val(v36*4),4,0))*1/ fi" now | sort -u -r > $TEMP_SELECTED
    echo >> $TEMP_SELECTED
    ls -l $TEMP_SELECTED
    ...
    $MX cipar=$FILE_CIPAR ISSUE btell=0 "seq=$PARM_ISSN$" lw=9999  "pft=if v36*0.4='$PARAM_SELECT_BY_YEAR' then if a(v131) and a(v132) and v32<>'ahead' and v32<>'review' and a(v41) then v35,v36*0.4,s(f(10000+val(v36*4),4,0))*1/ fi fi" now | sort -u -r > $TEMP_SELECTED
