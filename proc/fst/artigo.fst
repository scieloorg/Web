...
3 0 mpl,if v706='h' and a(v222) then 'SM=',v35,v65*0.4,s(f(val(v936^o)+10000,2,0))*1.4,'-',s(f(val(v121)+100000,1,0))*1.5/, fi,
3 0 mpl,if v706='old' then 'SM=',v35,s(f(val(v31)+1000,4,0))*1.3,s(f(val(v131)+1000,4,0))*1.3,s(f(val(v32)+1000,4,0))*1.3,s(f(val(v132)+1000,4,0))*1.3,'=',s(f(val(v121)+100000,1,0))*1.5 fi,
...
13 0 mpl,if v706='i' then if p(v41) then 'P' else if v32='ahead' or v32='review' then v32*0.1 else 'Y' fi,  fi,v35,v65*0.4,s(f(val(s(v36*4.4))+10000,2,0))*1.4/, fi,
13 0 mpl,if v706='i' and a(v41) then if v32='ahead' or v32='review' then 'Y' fi,v35,v65*0.4,s(f(val(s(v36*4.4))+10000,2,0))*1.4/, fi,
14 0 mpl,if v706='p' and p(v704^l) then 'TLN=',v880,v704^l fi,
15 0 mpl,if v706='h' then 'DTH=',v65*0.4, if v65*4.2 = '00' then, '01', else v65*4.2, fi, '01=',v35,s(f(val(v936^o)+10000,1,0))*1.4,s(f(val(v121)+100000,1,0))*1.5 fi,
...
90 0 mpl,if v706='h' then 'OAITS=',ref(mfn-1,if p(v93) then if val(v930.8) > val(v91) then v930.8 else v91 fi else v91 fi),'=',v35,s(f(val(v936^o)+10000,1,0))1.4,s(f(val(v121)+100000,1,0))1.5 fi, fi  /* oai-pmh - new and updated documents - SciELO.org */
