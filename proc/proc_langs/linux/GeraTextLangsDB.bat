...
if [ "@$1" == "@" ] then
	...
	export LANG_DB=$LANG_DATABASE_PATH/lang/lang
	export LANG_DB_OLD=$LANG_DATABASE_PATH/lang/replaced_lang

	if [ ! -f LANG_DB.mst ]
	then
	   mkdir -p $LANG_DATABASE_PATH/lang
	   $MX null count=0 create=$LANG_DB now
	fi
	$MX $LANG_DB fst=@proc_langs/fst/lang.fst fullinv=$LANG_DB

	if [ ! -f LANG_DB_OLD.mst ]
	then
	   mkdir -p $LANG_DATABASE_PATH/lang
	   $MX null count=0 create=$LANG_DB_OLD now
	fi
