...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="GetNumber">
		...
		<xsl:choose>
			...
			<xsl:when test="starts-with($num,'AHEAD')">ahead of print</xsl:when>
	...
	<xsl:template name="SHOWSTRIP">
		...
		<xsl:if test="$NUM and not(contains($NUM,'ahead'))">&#160;<xsl:value-of select="normalize-space($NUM)"/>
		</xsl:if>
...
-->
	<xsl:template name="CreateAuthor">
		...
		<!-- xsl:variable name="SEARCH_NOCHARS" select="translate($SEARCH,'é','E')" / -->
		...
		<xsl:choose>
			...
			<xsl:when test=" $SEARCH != '' ">
				<!-- a href="http://{$SERVER}{$PATH_WXIS}{$PATH_DATA_IAH}?IsisScript={$PATH_CGI_IAH}iah.xis&amp;base=article^dlibrary&amp;format={$NORM}.pft&amp;lang={$LANG_IAH}&amp;nextAction=lnk&amp;indexSearch=AU&amp;exprSearch={$SEARCH_NOCHARS}">xxxxx -->
