...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:template name="tool_box">
		<div id="toolBox">
			...
			<form name="addToShelf" method="post" action="http://{$SCIELO_REGIONAL_DOMAIN}/applications/scielo-org/services/addArticleToShelf.php" target="mensagem">
				<input type="hidden" name="PID" value="{/root/SERIAL/CONTROLINFO/PAGE_PID}"/>
				<input type="hidden" name="url" value="{concat('http://',/root/SERIAL/CONTROLINFO/SCIELO_INFO/SERVER,'/applications/scielo-org/scielo.php?script=sci_arttext&amp;pid=',/root/SERIAL/CONTROLINFO/PAGE_PID,'&amp;lng=',$LANGUAGE,'&amp;nrm=',/root/SERIAL/CONTROLINFO/STANDARD,'&amp;tlng=',/root/SERIAL/ISSUE/ARTICLE/@TEXTLANG)}"/>
			</form>
			...
			<ul>
				<xsl:choose>
					<xsl:when test="normalize-space(/root/USERINFO/@status) != normalize-space('logout') ">
						<li>
							<xsl:choose>
	...
	<xsl:template match="fulltext-service" mode="link">
		<xsl:variable name="params">
			<xsl:if test="@id='cited_Google'">,menubar=1,location=1,toolbar=1,status=1,scrollbars=1,directories=1</xsl:if>
		</xsl:variable>
	...
	<xsl:template match="fulltext-service[@id='cited']" mode="label">
	...
	<xsl:template match="fulltext-service[@id='cited_SciELO']" mode="label">
		<xsl:apply-templates select="//fulltext-service[@id='cited']" mode="label"/>
		SciELO
	</xsl:template>
