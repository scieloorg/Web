	...
	<xsl:template name="PrintExportCitationForRefecenceManagers">
		...
		<div class="serviceColumn">
			...
			<h3>
				<span>
					<xsl:choose>
						<xsl:when test=" $LANGUAGE = 'en' ">Others Formats</xsl:when>
						<xsl:when test=" $LANGUAGE = 'pt' ">Outros Formatos</xsl:when>
						<xsl:when test=" $LANGUAGE = 'es' ">Outros Formatos</xsl:when>
					</xsl:choose>
				</span>
			</h3>
			<ul>
				<li>
					<a href="?download&amp;format=XML&amp;pid={$pid}">
						<xsl:choose>
							<xsl:when test=" $LANGUAGE = 'en' ">XML Format</xsl:when>
							<xsl:when test=" $LANGUAGE = 'pt' ">Formato de XML</xsl:when>
							<xsl:when test=" $LANGUAGE = 'es' ">Formato de XML</xsl:when>
						</xsl:choose>
					</a>
				</li>
			</ul>
  ...
  <li> .
			...
			<xsl:choose>
				<xsl:when test=" $TYPE='abstract' ">
					...
					<xsl:choose>
						<xsl:when test=" $INTLANG = 'en' ">
							<xsl:if test="$FIRST_LABEL='1'">abstract in</xsl:if>
							<xsl:choose>
						...
						<xsl:when test=" $INTLANG = 'pt' ">
							<xsl:if test="$FIRST_LABEL='1'">resumo em</xsl:if>
							<xsl:choose>
						...
						<xsl:when test=" $INTLANG = 'es' ">
							<xsl:if test="$FIRST_LABEL='1'">resumen en</xsl:if>
							<xsl:choose>
				...
				<xsl:when test=" $TYPE='full' ">
					...
					<xsl:choose>
						...
						<xsl:when test=" $INTLANG = 'en' ">
							<xsl:if test="$FIRST_LABEL='1'">text in</xsl:if>
							<xsl:choose>
								<xsl:when test=" $TXTLANG = 'en' "> english</xsl:when>
								<xsl:when test=" $TXTLANG = 'pt' "> portuguese</xsl:when>
								<xsl:when test=" $TXTLANG = 'es' "> spanish</xsl:when>
						...
						<!-- fixed acrescenti first_label -->
						<xsl:when test=" $INTLANG = 'pt' ">
							<xsl:if test="$FIRST_LABEL='1'">texto em</xsl:if>
							<xsl:choose>
								<xsl:when test=" $TXTLANG = 'en' "> inglês</xsl:when>
								<xsl:when test=" $TXTLANG = 'pt' "> português</xsl:when>
								<xsl:when test=" $TXTLANG = 'es' "> espanhol</xsl:when>
						...
						<!-- fixed acrescenti first_label -->
						<xsl:when test=" $INTLANG = 'es' ">
							<xsl:if test="$FIRST_LABEL='1'">texto en</xsl:if>
							<xsl:choose>
								<xsl:when test=" $TXTLANG = 'en' "> inglés</xsl:when>
								<xsl:when test=" $TXTLANG = 'pt' "> portugués</xsl:when>
								<xsl:when test=" $TXTLANG = 'es' "> español</xsl:when>
				...
				<xsl:when test=" $TYPE='pdf' ">
					...
					<xsl:choose>
						<xsl:when test=" $INTLANG = 'en' ">
							<xsl:if test="$FIRST_LABEL='1'">pdf in
</xsl:if>
							<xsl:choose>
								<xsl:when test=" $TXTLANG = 'en' "> english</xsl:when>
								<xsl:when test=" $TXTLANG = 'pt' "> portuguese</xsl:when>
						...
						<xsl:when test=" $INTLANG = 'pt' ">
							<xsl:if test="$FIRST_LABEL='1'">pdf em</xsl:if>
							<xsl:choose>
								<xsl:when test=" $TXTLANG = 'en' "> inglês</xsl:when>
								<xsl:when test=" $TXTLANG = 'pt' "> português</xsl:when>
								<xsl:when test=" $TXTLANG = 'es' "> espanhol</xsl:when>
						...
						<xsl:when test=" $INTLANG = 'es' ">
							<xsl:if test="$FIRST_LABEL='1'">pdf en</xsl:if>
							<xsl:choose>
								<xsl:when test=" $TXTLANG = 'en' "> inglés</xsl:when>
								<xsl:when test=" $TXTLANG = 'pt' "> portugués</xsl:when>
								<xsl:when test=" $TXTLANG = 'es' "> español</xsl:when>
	...
	<xsl:template name="UpdateLog">
		<xsl:if test="//CONTROLINFO/SCIELO_INFO/SERVER_LOG!=''">
			<img>
				<xsl:attribute name="src">http://<xsl:value-of select="//CONTROLINFO/SCIELO_INFO/SERVER_LOG"/>/<xsl:value-of select="//CONTROLINFO/SCIELO_INFO/SCRIPT_LOG_NAME"/>?app=<xsl:value-of select="normalize-space(//CONTROLINFO/APP_NAME)"/>&amp;page=<xsl:value-of select="//CONTROLINFO/PAGE_NAME"/>&amp;<xsl:if test="//CONTROLINFO/PAGE_PID">pid=<xsl:value-of select="//CONTROLINFO/PAGE_PID"/>&amp;</xsl:if>lang=<xsl:value-of select="normalize-space(//CONTROLINFO/LANGUAGE)"/>&amp;norm=<xsl:value-of select="normalize-space(//CONTROLINFO/STANDARD)"/>&amp;doctopic=<xsl:value-of select="//ARTICLE/@DOCTOPIC"/></xsl:attribute>
				<xsl:attribute name="border">0</xsl:attribute>
...
-->
	...
	<xsl:template match="LATTES">
		...
		<xsl:choose>
			<xsl:when test=" count(AUTHOR) = 1 ">
				...
				<a href="{AUTHOR/@HREF}" onmouseover="status='{AUTHOR/@HREF}'; return true;" onmouseout="status='';" style="text-decoration: none;">
					<font class="nomodel" size="2">CV ScienTI</font>
				</a>
			</xsl:when>
			<xsl:when test=" count(AUTHOR) > 1 ">
	...
	<!-- 
	revisado PrintArticleInformationLink
	-->
	<xsl:template name="PrintArticleInformationLink">
		...
		<a href="javascript:void(0);" onmouseout="status='';" class="nomodel" style="text-decoration: none;">
			<xsl:attribute name="onclick">OpenArticleInfoWindow ( 850, 500,  "<xsl:value-of select="$INFOPAGE"/>");</xsl:attribute>
			<xsl:attribute name="onmouseover">
				status='<xsl:call-template name="PrintArticleInformationLabel"><xsl:with-param name="LANGUAGE" select="$LANGUAGE"/></xsl:call-template>'; return true;
			</xsl:attribute>
...
<xsl:template match="USERINFO" />
	<xsl:template name="PrintArticleReferenceElectronicVancouver">
		...
		<xsl:if test="substring($SHORTTITLE,string-length($SHORTTITLE)) != '.' ">.</xsl:if>&#160;
		<xsl:choose>
			<xsl:when test=" $LANG = 'en' ">[serial in the Internet]. </xsl:when>
	...
	<xsl:template match="AUTHORS" mode="ref">
		...
		<xsl:variable name="MAX_PRESENTED"><xsl:choose>
			<xsl:when test="count(.//AUTH_PERS/AUTHOR)&gt;$max"><xsl:value-of select="$max_presented"/></xsl:when>
