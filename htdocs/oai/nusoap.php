...
class soap_parser extends nusoap_base {
	...
	*/
	function character_data($parser, $data){
		...
		// Test data level against root struct level
		if ( $this->root_struct == $pos )
		{
			// No parameter is passed (no child element of root struct)
      		$this->message[$pos]['cdata'] .= "";
		}
		else
		{
			// There is at least one child element of root (parameter name and type)
			$this->message[$pos]['cdata'] .= $data;
		}
        // for doclit
        if($this->status == 'header'){
