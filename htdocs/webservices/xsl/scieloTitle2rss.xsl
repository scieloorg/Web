...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<rss version="0.91">
			<channel>
				...
				<xsl:choose>
					<xsl:when test="$letter != ''"><xsl:apply-templates select="//field[@tag='100']/occ[substring(.,1,1)=$letter]" mode="getItem"/></xsl:when>
					<xsl:otherwise><xsl:apply-templates select="//field[@tag='100']/occ" mode="getItem"/></xsl:otherwise>
				</xsl:choose>
			</channel>
		</rss>		
	</xsl:template>
	...
	<xsl:template match="*" mode="getItem">
		<xsl:variable name="pid" select="../../field[@tag='400']/occ"/>
		<xsl:variable name="sta" select="../../field[@tag ='50']/occ"/>		
		<xsl:if test="$sta = 'C'">
			<item>
