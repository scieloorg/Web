...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mml="http://www.w3.org/1998/Math/MathML">
			...
			<xsl:otherwise>
				...
				<xsl:choose>
					<xsl:when test="$article_lang='pt'">
						<li class="nav-ul-li">REFERÊNCIAS</li>
					</xsl:when>
	...
	<xsl:template match="table-wrap/graphic" mode="tooltip-body">
		<div id="t{@id}" class="atip"><img class="fig"><xsl:apply-templates select="@xlink:href" mode="src"/></img></div>
	...
	<xsl:template match="ref" mode="tooltip-body-ref">
		<div id="t{@id}" class="atip">
			<div class="xref-ref">
				...
				<xsl:if test=".//label = false()">
					<xsl:value-of select="position()"/>.&#160;
				</xsl:if>
				<xsl:apply-templates select="element-citation | mixed-citation | citation | nlm-citation"/>
			</div>
		</div>
	</xsl:template>
	...
	<!--xsl:template match="body/text()"> 		<xsl:value-of select="."/> 	</xsl:template-->
	...
	<xsl:template match="contrib-group">
		<div class="autores">
			<xsl:apply-templates select="*"/>
						<xsl:apply-templates select="aff" mode="del-aff"/>
		</div>
	</xsl:template>
	<xsl:template match="aff" mode="del-aff"/>
		<!--imprime e ordena os autores-->
	<xsl:template match="contrib">
		<xsl:choose>
			<xsl:when test="position()=last() and count(..//contrib)&gt;1">
				and
			</xsl:when>
	...
	<xsl:template match="graphic" mode="img-div">
		<img class="fig"><xsl:apply-templates select="@xlink:href" mode="src"/></img>
	...
	<xsl:template match="table-wrap/graphic">
		<img class="figure"><xsl:apply-templates select="@xlink:href" mode="src"/></img>
	</xsl:template>
	...
	<xsl:template match="ref">
		<p class="ref">
			...
			<xsl:if test="./label">	<xsl:apply-templates select="label"/>.&#160;</xsl:if>
			<xsl:if test=".//label = false()"><xsl:value-of select="position()"/>.&#160;</xsl:if>
			<xsl:apply-templates select="element-citation | mixed-citation | citation | nlm-citation"/>
			</a>
		</p>
	</xsl:template>
