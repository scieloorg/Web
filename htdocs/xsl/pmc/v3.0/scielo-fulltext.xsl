...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="refpos">
		<xsl:choose>
			<xsl:when test="$xml_article">
				<xsl:apply-templates select="document($xml_article)//ref" mode="position"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select=".//ref" mode="position"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:template match="ref" mode="position">{<xsl:value-of select="@id"/>}<xsl:value-of
			select="position()"/>{/<xsl:value-of select="@id"/>}</xsl:template>
	<xsl:template match="ref" mode="get_position">
	...
	<xsl:template match="article" mode="text-content">
		...
		<xsl:apply-templates
			select="front//article-meta//abstract | sub-article[@article-type='translation']//abstract"/>
	...
	<xsl:template match="contrib">
		...
		<xsl:if test="xref">
			<xsl:apply-templates select="." mode="xref-list"/>
		</xsl:if>
	...
	<xsl:template match="date" mode="en">
		...
		<xsl:apply-templates select="day"/>, <xsl:apply-templates select="year"/>
		<xsl:choose>
	...
	<xsl:template match="date" mode="pt">
		<xsl:choose>
			<xsl:when test="@date-type='received'"> Recebido em </xsl:when>
			<xsl:when test="@date-type='accepted'"> Aceito em </xsl:when>
		</xsl:choose>
		<xsl:apply-templates select="day"/> de <xsl:apply-templates select="month" mode="date-pt"/>
		de <xsl:apply-templates select="year"/>
		<xsl:choose>
			<xsl:when test="@date-type='received'">;</xsl:when>
			<xsl:when test="@date-type='accepted'">.</xsl:when>
		</xsl:choose>
	...
	<xsl:template match="date" mode="es">
		<xsl:choose>
			<xsl:when test="@date-type='received'"> Recebido el </xsl:when>
			<xsl:when test="@date-type='accepted'"> Aceptado el </xsl:when>
		</xsl:choose>
		<xsl:apply-templates select="day"/> de <xsl:apply-templates select="month" mode="date-es"/>
		de <xsl:apply-templates select="year"/>
		<xsl:choose>
			<xsl:when test="@date-type='received'">;</xsl:when>
			<xsl:when test="@date-type='accepted'">.</xsl:when>
	...
	<xsl:template match="@* | text()" mode="debug"> |<xsl:value-of select="."/>| </xsl:template>
	<!--Lilsta as palavras chave dentro de Abstract-->
	...
	<xsl:template match="graphic | disp-formula//graphic">
		<a target="_blank">
			<xsl:apply-templates select="@xlink:href" mode="href"/>
			<img class="graphic">
				<xsl:apply-templates select="@xlink:href" mode="src"/>
			</img>
		</a>
	</xsl:template>
	...
	<xsl:template match="fig">
		<div class="figure">
			...
			<div class="label_caption">
				<xsl:apply-templates select="label"/>
				<xsl:if test="label and caption"> - <xsl:apply-templates select="caption"/>
				</xsl:if>
			</div>
	...
	<!--SigBlock-->
	<!--sig-block>             <sig>Joel <bold>FAINTUCH</bold>                 <sup>1</sup>             </sig>             <sig>Ricardo Guilherme <bold>VIEBIG</bold>                 <sup>2</sup>             </sig> 		</sig-block-->
		...
		<p class="ref">
			<a name="{@id}"><!--Imprime o label das referências-->
				...
				<xsl:if test="not(.//label)">
					<xsl:value-of select="position()"/>.&#160; <xsl:choose>
						<xsl:when test="mixed-citation">
							<xsl:apply-templates select="mixed-citation"/>
						</xsl:when>
						<xsl:when test="element-citation">
			...
			<xsl:variable name="aref">000000<xsl:apply-templates select="." mode="get_position"
				/></xsl:variable>
	...
	<!--Lista de autores,tanto mixed como element citation-->
	<!--xsl:template match="person-group" mode="mixed-content"> 		<xsl:apply-templates select="name | collab" mode="list-author-ref"/>     </xsl:template-->
	...
	<xsl:template match="name" mode="element-content">
		<span class="ref-autor">
			...
			<xsl:if test="position() = last()">
				<xsl:choose>
					<xsl:when test="..//etal"/>
					<xsl:when test="../@person-group-type='transed'">,translator and
						editor.</xsl:when>
					<xsl:when test="../@person-group-type != 'author'">
						<xsl:choose>
							<xsl:when test="position() = 1 and position() = last()">, <xsl:value-of
									select="../@person-group-type"/>. </xsl:when>
							<xsl:otherwise>,<xsl:value-of select="../@person-group-type"
								/>s.</xsl:otherwise>
						</xsl:choose>
					</xsl:when>
		...
		<span class="ref-autor">
			...
			<xsl:if test="@collab-type">
				<xsl:choose>
					...
					<xsl:otherwise>,<xsl:value-of select="@collab-type"/>s </xsl:otherwise>
				</xsl:choose>
			</xsl:if>
			<xsl:if test="position() = last()">
				<xsl:choose>
	...
	<xsl:template match="string-name" mode="element-content">
		<span class="ref-autor">
			<xsl:if test="position() &gt; 1">; </xsl:if>
			<xsl:apply-templates select="* | text()"/>
			<xsl:if test="suffix"> &#160; <xsl:apply-templates select="suffix"/>
			</xsl:if>
			<xsl:if test="position() = last()">
				<xsl:choose>
					<xsl:when test="..//etal">
	...
	<xsl:template match="aff" mode="element-content">(<xsl:apply-templates/> ) </xsl:template>
	<xsl:template match="article-title | chapter-title" mode="element-content">
		<span class="ref-title">&#160; <xsl:apply-templates/>
			<xsl:choose>
	...
	<xsl:template match="trans-title | trans-source" mode="element-content">
		<span class="ref-title-t">
			<xsl:choose>
				<xsl:when test="contains(.,'[')">&#160; <xsl:apply-templates/>.</xsl:when>
				<xsl:otherwise>&#160;[ <xsl:apply-templates/> ]. </xsl:otherwise>
			</xsl:choose>
		</span>
	</xsl:template>
	<xsl:template match="source" mode="element-content">
		<span class="ref-source">&#160; <xsl:apply-templates/>
			<xsl:choose>
	...
	<xsl:template match="patent" mode="element-content"> &#160;<xsl:value-of select="."/>. </xsl:template>
	<xsl:template match="edition" mode="element-content">
		<xsl:choose>
			<xsl:when
				test="../@publication-type='journal' or ../@publication-type='newspaper' or ../@citation-type='journal'  or ../@citation-type='newspaper'"
				>(<xsl:apply-templates/> ). </xsl:when>
			<xsl:when
				test="../@publication-type='book' or ../@publication-type='report' or ../@citation-type='book' or ../@citation-type='report'"
				>&#160; <xsl:apply-templates/>.</xsl:when>
		</xsl:choose>
	</xsl:template>
	<xsl:template match="publisher-loc" mode="element-content">
		<xsl:choose>
			<xsl:when test="../@publication-type='journal'">(<xsl:apply-templates/> ). </xsl:when>
			<xsl:when test="../@publication-type='book'">&#160; <xsl:apply-templates/> : </xsl:when>
		</xsl:choose>
	</xsl:template>
	<xsl:template match="publisher-name" mode="element-content">&#160;<xsl:apply-templates/> ; </xsl:template>
	...
	<xsl:template match="conf-loc" mode="element-content"
		>&#160;<xsl:apply-templates/>.</xsl:template>
	<xsl:template match="year" mode="element-content">
		<span class="data">&#160; <xsl:apply-templates/>
			<xsl:choose>
	...
	<xsl:template match="issue-title" mode="element-content"
		>&#160;<xsl:apply-templates/>):</xsl:template>
	...
	<xsl:template match="size" mode="element-content"> &#160;<xsl:value-of select="."/>. </xsl:template>
	<xsl:template match="series" mode="element-content">
		<span class="data">&#160;(<xsl:value-of select="."/>).</span>
	</xsl:template>
	<xsl:template match="date-in-citation" mode="element-content">
		<xsl:choose>
			<xsl:when test="@content-type='epub'">&#160; <xsl:apply-templates/>.</xsl:when>
			<xsl:otherwise> [ <xsl:apply-templates/> ]; </xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<xsl:template match="etal" mode="element-content"> &#160;<i>et al.</i>
	</xsl:template>
	<xsl:template match="comment" mode="element-content">&#160;<xsl:apply-templates/>
	</xsl:template>
	<xsl:template match="isbn" mode="element-content"> &#160;ISBN:<xsl:value-of select="."/>. </xsl:template>
	<xsl:template match="uri"> &#160;<a href="{.}" target="_blank"><xsl:value-of select="."/></a>
	</xsl:template>
	...
	<xsl:template match="pub-id" mode="element-content">
		<xsl:choose>
			<xsl:when test="@pub-id-type='pmid'">&#160;PMID<a
					href="http://www.ncbi.nlm.nih.gov/pubmed/{normalize-space(.)}" target="_blank"
						><xsl:value-of select="."/></a>
			</xsl:when>
	...
	<!--   Fim das referências 3.0   -->
	<!--     ************************************     Referências na versão citation (2.?)     ************************************     -->
	<!--     **********************     Fim de referências 2.?     **********************     -->
	<!--Notas do Artigo [foot notes]-->
	<xsl:template match="fn-group">
		<div>
			<a name="footnotes-nav">
				<xsl:choose>
