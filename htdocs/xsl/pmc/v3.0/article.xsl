...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="@*" mode="labeled"><xsl:value-of select="."/></xsl:template>
    <xsl:template match="@*[.='ppub']" mode="labeled">Print</xsl:template>

    <xsl:template match="issn" mode="labeled">
        <p class="{name()}"><xsl:apply-templates select="@*" mode="labeled"/> version ISSN <xsl:value-of select="."/></p>
    </xsl:template>
    ...
    <xsl:template match="article" mode="text-header">
        ...
        <p class="journal-title"><a><xsl:apply-templates select="." mode="journal-link"/><xsl:value-of select=".//journal-meta//journal-title" /></a></p>

        <xsl:apply-templates select=".//journal-meta/issn" mode="labeled"/>
        
        <p class="bib-strip">
            <xsl:value-of select=".//journal-meta//abbrev-journal-title"/>&#160;
            <xsl:value-of select=".//article-meta//volume"/><xsl:if test=".//article-meta/issue">
                (<xsl:value-of select=".//article-meta/issue"/>)
            </xsl:if>:
            
            <xsl:value-of select=".//article-meta/fpage"/>
            <xsl:if test=".//article-meta/lpage">-<xsl:value-of select=".//article-meta/lpage"/>
        </xsl:if>, <xsl:apply-templates select=".//article-meta/pub-date" mode="bib-strip"/>.
        </p>
    ...
    <xsl:template match="article" mode="sections-navegation">
        <div id="nav">
            <ul id="nav">
                ...
                <xsl:apply-templates select=" .//article-meta/abstract | .//article-meta/trans-abstract" mode="sections-navegation"/>
                <!--Lista e cria o link para os títulos de seções no artigo e Referências-->
                <xsl:apply-templates select=".//body/sec/title | .//back/ref-list/title | .//back/ack/sec/title" mode="sections-navegation"/>

            </ul>
        </div>
    </xsl:template>
    <xsl:template match="abstract | trans-abstract" mode="sections-navegation">
    ...
    <xsl:template match="ref" mode="tooltip-body">
          <div id="t{@id}" class="atip">
          	<div class="xref-ref">
                  ...
                  <xsl:apply-templates select="element-citation | citation | mixed-citation"/>.
             </div>
          </div>
     </xsl:template>
     ...
     <xsl:template match="fig" mode="tooltip-body">
     	<div class="atip" id="t{@id}">
          <div class="xref-img"><xsl:apply-templates select="@* | * | text()"/>
    ...
    *********************************
    ...
    <xsl:template match="contrib">
         <xsl:choose>
            <xsl:when test="position()=last() and count(..//contrib)&gt;1"> and </xsl:when>
...
<!--Fim de Notas de autor-->
    ...
    ********************************************
    ...
    <xsl:template match="@* | text()" mode="debug">|<xsl:value-of select="."/>|
    </xsl:template>
    <!--Lilsta as palavras chave dentro de Abstract-->
...
<!--Exibe uma subseção dos artigos-->
    ...
    <xsl:template match="fig">
        <div class="xref-img">
          <a name="{@id}"> <xsl:apply-templates select="@* | * | text()"/>
              <xsl:apply-templates select="graphic" mode="img-div"/>
