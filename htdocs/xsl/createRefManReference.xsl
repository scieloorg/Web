...
|'"/>	
	...
	<xsl:template name="getTitle">
		<xsl:if test="/SERIAL/ARTICLE/TITLES/TITLE != '' ">
			<xsl:value-of select="'T1  - '" disable-output-escaping="yes"/>
	...
	<xsl:template name="getJournal">
		<xsl:if test="/SERIAL/TITLEGROUP/TITLE != '' ">
			<xsl:value-of select="'JO  - '" disable-output-escaping="yes"/>
	...
	<xsl:template name="getAutors">
		...
		<xsl:for-each select="/SERIAL/ARTICLE/AUTHORS/AUTH_PERS/AUTHOR">
			<xsl:value-of select="'A1  - '" /><xsl:value-of select="SURNAME" disable-output-escaping="no"/>, <xsl:value-of select="NAME" disable-output-escaping="no"/>
			<xsl:call-template name="space"/>
		</xsl:for-each>
	</xsl:template>
	...
	<xsl:template name="getISSN">
		<xsl:if test="/SERIAL/ISSN != ''">
			<xsl:value-of select="concat('SN  - ', /SERIAL/ISSN,'')"/>
			<xsl:call-template name="space" />
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="getURL">
		<xsl:if test="/SERIAL/CONTROLINFO/SCIELO_INFO/SERVER != ''">
			<xsl:value-of select="concat('UL  - http://',/SERIAL/CONTROLINFO/SCIELO_INFO/SERVER,'/scielo.php?script=sci_arttext&amp;pid=',//SERIAL/CONTROLINFO/PAGE_PID,'&amp;nrm=iso')"/>
			<xsl:call-template name="space" />
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="getVol">
		<xsl:if test="/SERIAL/ISSUEINFO/@VOL != ''">
			<xsl:value-of select="concat('VL  - ',/SERIAL/ISSUEINFO/@VOL,'')" />
			<xsl:call-template name="space" />
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="getYear">
		<xsl:if test="/SERIAL/ISSUEINFO/@YEAR != '' ">
			<xsl:value-of select="concat('Y1  - ',/SERIAL/ISSUEINFO/@YEAR,'')" />
			<xsl:call-template name="space" />
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="getPages">
		<xsl:if test="/SERIAL/ARTICLE/@FPAGE != ''">
			...
			<xsl:value-of select="concat('SP  - ', /SERIAL/ARTICLE/@FPAGE)" />
			<xsl:call-template name="space" />
			<xsl:value-of select="concat('EP  - ', /SERIAL/ARTICLE/@LPAGE)" />
			<xsl:call-template name="space" />
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="getPublisher">
		<xsl:if test="/SERIAL/CONTROLINFO/APP_NAME != ''">
			<xsl:value-of select="concat('PB  - ',/SERIAL/CONTROLINFO/APP_NAME,'')" />
			<xsl:call-template name="space" />
		</xsl:if>
	</xsl:template>
