		...
		<html>
			<head>
				<title>
					...
					<xsl:value-of select="$translations/xslid[@id='sci_pdf']/text[@find = '-pdf_download-']"/>
					<xsl:value-of select="ARTICLE/@PID"/>
				</title>
				...
				<table cellspacing="0" border="0" cellpadding="7" width="100%">
					<tr>
						<td width="26%" valign="TOP">
							<p align="CENTER">
								<a>
									<xsl:attribute name="href">http://<xsl:value-of select="CONTROLINFO/SCIELO_INFO/SERVER"/><xsl:value-of select="CONTROLINFO/SCIELO_INFO/PATH_DATA"/>scielo.php/lng_<xsl:value-of select="CONTROLINFO/LANGUAGE"/></xsl:attribute>
									<img>
						...
						<td width="80%">
							<p align="LEFT">
								<font size="3" color="#000080">
									<b>
										<xsl:value-of select="$translations/xslid[@id='sci_pdf']/text[@find = 'sentence_1']"/>
									</b>
								</font>
							</p>
